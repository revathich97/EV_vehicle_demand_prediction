...
st.markdown("""
    <style>
        ...
        .stApp {
            background: linear-gradient(to bottom, #1E1E2F, #1C1C1C);
            padding: 20px;
        }
        h1, h2, h3, h4 {
...
# === Compare up to 5 counties ===
st.markdown("---")
st.header("Compare EV Adoption Trends for up to 5 Counties")

# Change the max_selections parameter to 5
...
if multi_counties:
    ...
    for cty in multi_counties:
        ...
        future_rows_cty = []
        forecast_horizon = 36 # Keep the forecast horizon consistent
    ...
    for cty in multi_counties:
        ...
        # Find the last historical cumulative value. The forecast starts after this point.
        historical_total = cty_df['Cumulative EV'].iloc[len(cty_df) - forecast_horizon - 1]
        # The forecasted total is the last value in the dataframe.
    ...
    st.success(f"Forecasted EV adoption growth over the next 3 years — {growth_sentence}")


st.markdown("Prepared by **Revathi** for AICTE Internship")